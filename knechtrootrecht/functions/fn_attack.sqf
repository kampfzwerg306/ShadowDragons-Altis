...
if (!hasInterface)then
{
	...
	while{(alive _me)and(count _myTarget != 0)}do
	{
		...
		if((_distance<=20)and(!_imHere))then
		{
			...
			[[_me,"idle"],'INF_fnc_NextSound',true,false] spawn life_fnc_MP;
		};
		if(_distance<=2.5)then
		{	
			if ((alive _me) and (alive _target))then
			{
				//_me switchMove "AmovPercMrunSlowWpstDf_AmovPercMstpSrasWpstDnon_gthThrow";
				[[_me,'AwopPercMstpSgthWnonDnon_end'],'life_fnc_animSync',true,false] spawn life_fnc_MP;
				_target setDamage (damage _target + ((random 50)/100));
				//[_me, "punch"] spawn INF_fnc_NextSound;
				[[_me,"punch"],'INF_fnc_NextSound',true,false] spawn life_fnc_MP;
				sleep 3;
				[[_me,'AfalPknlMstpSnonWnonDnon'],'life_fnc_animSync',true,false] spawn life_fnc_MP;
				//_me switchMove "AwopPercMstpSgthWpstDnon_Part1";
