...
if(!hasInterface)then
{
	...
	if(_indoor)then
	{
					...
					if (!(typeOf _house in _exclusionList))then
					{
						for "_n" from 0 to 50 do {
							...
							if (_probability > random 100)then
							{
								if(_horde)then
								{
									"O_G_Soldier_M_F" createUnit [ _buildingPos, infected,"enableRadio false;nok_zombi = this", random 1, "corporal"];
									_object = nok_zombi;
									//(_object select 0) switchMove 'AfalPknlMstpSnonWnonDnon';
									[[_object,'AfalPknlMstpSnonWnonDnon'],'life_fnc_animSync',true,false] spawn life_fnc_MP;
		...
		while{_totalInfected>=_count}do
		{
			...
			if(_horde)then
			{
				"O_G_Soldier_M_F" createUnit [ _markPos, infected,"enableRadio false;nok_zombi = this", random 1, "corporal"];
				_object = nok_zombi;
				//(_object select 0) switchMove 'AfalPknlMstpSnonWnonDnon';
				[[_object,'AfalPknlMstpSnonWnonDnon'],'life_fnc_animSync',true,false] spawn life_fnc_MP;
				[_object,_myZone] spawn INF_fnc_initHordeAttacker;
				...
				"O_G_Soldier_M_F" createUnit [ _markPos, infected,"enableRadio false;nok_zombi = this", random 1, "corporal"];
				_object = nok_zombi;
				//[_markPos, 180, "O_soldier_F", infected] call bis_fnc_spawnvehicle;
				//(_object select 0) switchMove 'AfalPknlMstpSnonWnonDnon';
				[[_object,'AfalPknlMstpSnonWnonDnon'],'life_fnc_animSync',true,false] spawn life_fnc_MP;
				[_object,_myZone] spawn INF_fnc_infectedinit;
			};
