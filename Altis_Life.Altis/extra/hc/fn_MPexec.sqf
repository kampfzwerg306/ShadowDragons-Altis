...
if(!(["life_fnc_",_functionName] call BIS_fnc_inString) && {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && {!(["HC_fnc_",_functionName] call BIS_fnc_inString)} && {!(["INF_fnc_",_functionName] call BIS_fnc_inString)} && {!(["nok_fnc_",_functionName] call BIS_fnc_inString)} && {!(["TON_fnc_",_functionName] call BIS_fnc_inString)} &&
...
if (ismultiplayer && _mode == 0) then {
	if (!hasinterface) then {
		if (typename _target == typename []) then {
			...
			//--- Multi execution
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call hc_fnc_MPexec;
			} foreach _target;
		} else {
			...
			hc_fnc_MP_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
			...
			if (_ownerID < 0 || _ownerID == _serverID) then {
				["hc_fnc_MP_packet",hc_fnc_MP_packet] spawn hc_fnc_MPexec;
			};
