...
with missionnamespace do {
	...
	if(!hasinterface) then {
		hc_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];
	} else {
		hc_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall,profileName,getPlayerUID player];
	};
	HC2_owner publicvariableclient "HC_fnc_MP_packet";

	//--- Local execution
	if !(ismultiplayer) then {
		["hc_fnc_MP_packet",hc_fnc_MP_packet] spawn life_fnc_MPexec;
	};
