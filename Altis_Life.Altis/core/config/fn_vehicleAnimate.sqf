...
//diag_log "CALLING VEHICLE ANIMATE";
if(!_preset) then
{
	...
	else
{
	switch (_animate) do
	{
		...
		case "cop_offroad":
		{
			//diag_log "CALLING VEHICLE ANIMATE COP CRAP";
			if(typeOf _vehicle == "C_Offroad_01_F") then {
				waitUntil {!isNil {_vehicle getVariable "Life_VEH_color"}};
			};
			_vehicle animate ["HidePolice", 0];
		...
		case "five0_offroad":
		{
		diag_log "YEAH FIVE-0 OFFROAD";
			//HideBackpacks, HideBumper1, HideBumper2, HideConstructio
