...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if (playerside == west) then {
		[[0,format[localize "STR_NOTF_BeingImpounded_police_perm",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	};
	if (playerside == east) then {
		[[0,format[localize "STR_NOTF_BeingImpounded_adac_perm",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	};
	if (playerside == civilian) then {
		[[0,format[localize "STR_NOTF_BeingImpounded_sd_perm",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	};
		

	life_action_inUse = true;
	
	_upp = localize "STR_NOTF_Impounding_perm";
