...
//Error check
if(isNull _vendor OR _type == "" OR (player distance _vendor > 10)) exitWith {_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};

//unprocessed item,processed item, cost if no license,Text to display (I.e Processing  (percent) ..."
_itemInfo = switch (_type) do
{
	case "oil": {["oil","oilp",1200,(localize "STR_Process_Oil")];};
    ...
    case "tabak": {["tabak","zigaretten",250,"Verarbeite Tabak zu Zigaretten"]};
    case "zigarren": {["tabak","zigarren",450,"Verarbeite Tabak zu Zigarren"]};
	case "mohnbrötchen": {["keksu","keksp",450,"Verarbeite Mehl zu Mohnbrötchen"]};
	default {[];};
};

//Error checking
if(count _itemInfo == 0) exitWith {_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
...
_cost = _cost * _oldVal;
//Some more checks
if(_oldVal == 0) exitWith {_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
