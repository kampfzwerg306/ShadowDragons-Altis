...
fnc_setupFuncs = {
    ...
    life_publicVarList = ["shadowos","shadowos_inventory","shadowos_licenses","shadowos_money"];


    if(!isServer) then {
        diag_log "::Life Client:: Loading Smart Phone";


        //Tell player what we are doing on the menu screen.
        cutText["Loading Shadow Os 1.0, please wait","BLACK FADED"];
        {
